...
import { User, Award, Users, Tag, ArrowLeft, Send, Receipt } from "lucide-react";
...
export default function RegistrationSummary({
  ...
  const totalParticipants = isOlympiad ? 1 : 1 + teamMembers.length;
    ...
    <div className="space-y-8 animate-fadeIn">
      ...
      <div className="text-center max-w-2xl mx-auto space-y-2.5">
        ...
        <h2 className="text-3xl sm:text-4xl font-black text-white tracking-tight">
          Review <span className="gradient-text">Summary & Payment</span>
        </h2>
        <p className="text-slate-300 text-sm font-medium">
          Please verify your details and complete payment to confirm your FIRSO Rongpur registration.
        </p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">

        {/* Left Column: Summary Cards & Promo Code Box */}
        <div className="lg:col-span-7 space-y-6">
          ...
          <div className="glass-panel p-6 sm:p-7 rounded-3xl border border-red-500/30 space-y-4 shadow-xl">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 text-xs">
              <div>
                <span className="text-slate-400 block mb-1 font-medium">Selected Event / Subcategory:</span>
                <span className="font-extrabold text-white text-sm">
                  {isOlympiad
                    ? registration.subcategories?.length === 2
                      ? "Both Mathematics & Science Olympiads (Combo)"
                      : registration.subcategories?.map((s) => (s === "math" ? "Mathematics Olympiad" : "Science Olympiad")).join(", ") || "Olympiad"
                    : registration.competition || "N/A"}
                </span>
              </div>

              <div>
                <span className="text-slate-400 block mb-1 font-medium">Age Group:</span>
                <span className="font-extrabold text-white text-sm">{registration.ageGroup || "N/A"}</span>
              </div>

              {!isOlympiad && (
                <div>
                  <span className="text-slate-400 block mb-1 font-medium">Team Name:</span>
                  <span className="font-extrabold text-red-400 text-sm">{registration.teamName || "Solo Leader"}</span>
                </div>
              )}

              <div>
                <span className="text-slate-400 block mb-1 font-medium">Total Participants:</span>
                <span className="font-extrabold text-white text-sm">{totalParticipants} Participant(s)</span>
              </div>
            </div>
          </div>
          ...
          <div className="glass-panel p-6 sm:p-7 rounded-3xl border border-red-500/30 space-y-4 shadow-xl">
            <div className="border-b border-white/10 pb-3.5">
              <h3 className="text-sm font-black uppercase tracking-wider text-red-400 flex items-center gap-2">
                ...
                {isOlympiad ? "Participant Details" : "Team Leader Details"}
              </h3>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
              ...
              <div>
                ...
                <span className="font-semibold text-slate-200">{leader.email}</span>
              </div>
              <div>
                <span className="text-slate-400 block font-medium">Phone / WhatsApp:</span>
                <span className="font-semibold text-slate-200">{leader.phone}</span>
              </div>
              <div>
                <span className="text-slate-400 block font-medium">Educational Institution:</span>
                <span className="font-semibold text-slate-200">{leader.institution} ({leader.class})</span>
              </div>
              <div>
                <span className="text-slate-400 block font-medium">T-Shirt Size:</span>
                <span className="font-extrabold text-red-400 text-sm">{leader.tshirtSize}</span>
              </div>
            </div>
          </div>
            ...
            <div className="glass-panel p-6 sm:p-7 rounded-3xl border border-red-500/30 space-y-4 shadow-xl">
              ...
              <div className="space-y-3">
                {teamMembers.map((m, idx) => (
                  <div key={idx} className="glass-card p-3.5 rounded-2xl border border-white/5 flex items-center justify-between text-xs">
                    <div>
                      <div className="font-extrabold text-white">{idx + 2}. {m.name}</div>
                      <div className="text-slate-400 font-medium">{m.institution} ({m.class}) • {m.phone}</div>
                    </div>
            ...
            <div className="glass-panel p-4 sm:p-5 rounded-2xl border border-red-500/40 bg-red-950/20 flex items-center justify-between text-xs">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <span className="font-black text-red-300 uppercase tracking-wider">{registration.referralCodeEntered}</span>
                </div>
              </div>
              <div className="text-slate-400 font-medium">
                Division: <span className="font-bold text-white">Rongpur</span>
              </div>
            </div>
          )}

          {/* 👈 PROMO CODE INPUT CARD MOVED TO LEFT COLUMN */}
          <PromoCode promo={registration.promo} onApplyPromo={handleApplyPromo} />

        </div>

        {/* Right Column: Fee Breakdown & Payment Form */}
        <div className="lg:col-span-5 space-y-6">
          ...
          <div className="glass-panel p-6 sm:p-7 rounded-3xl border-2 border-red-500/50 space-y-5 bg-gradient-to-b from-slate-950/95 via-slate-950 to-red-950/50 shadow-2xl shadow-red-600/25">
            ...
            <div className="space-y-3 text-xs">
              <div className="flex items-center justify-between text-slate-300">
                <span>Original Fee ({totalParticipants} Participant/s):</span>
                <span className="font-bold text-white text-sm">{feeDetails.originalFee} BDT</span>
              </div>

              {feeDetails.discountAmount > 0 && (
                <div className="flex items-center justify-between text-emerald-400 font-semibold bg-emerald-500/10 p-2.5 rounded-xl border border-emerald-500/20">
                  <span>Discount ({feeDetails.discountPercent}% OFF):</span>
                  <span>- {feeDetails.discountAmount} BDT</span>
                </div>
              )}

              <div className="pt-4 border-t border-white/15 flex items-center justify-between">
                <span className="text-sm font-black uppercase text-white">Payable Amount:</span>
          ...
          {/* bKash / Nagad Payment Card */}
          <PaymentSection
            payment={registration.payment}
            onChange={(pay) => onChange({ payment: pay })}
            errors={errors}
          />
